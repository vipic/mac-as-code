...
进入菜单时清屏一次，移动光标时整帧缓冲后原位覆盖，不再清空整页。顶部为项目说明，中间为操作区，底部显示当前按键。单选使用 `↑↓`（或 `j` / `k`）移动、`Enter` 执行；`b` 返回上一层，`q` 退出，无需回车。确认页先展示影响说明，再在页面下方固定显示具体确认问题及 `y` / `Enter` 将执行的操作；`b` 返回、`q` 退出。详情和长确认内容支持方向键与 PageUp / PageDown 滚动。返回和退出不会撤销已经完成的操作。完整层级见[交互操作树](docs/操作树.md)。
...
- Homebrew formula 和 cask 的安装清单各读取一次，再与配置逐项匹配；批量清单未命中的名称单独复核，以保留别名识别。任一清单读取失败会停止检测，不把软件误报为未安装。
...
- 调整界面按系统设置、Dock 和软件来源标明类型并分隔；选择「全部」时翻页也会重复当前类型标题。「查看详情」按相同类型分段，并标出需确认项。界面支持 `↑↓` / `j k` 移动、空格切换、`a` 全选、`n` 清空；`Enter` 保存选择返回，`b` 放弃此次调整返回范围选择，`q` 退出程序。长列表随光标翻页。
